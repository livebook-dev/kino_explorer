...
export function init(ctx, payload) {
  ...
  const app = Vue.createApp({
        ...
        <form @change="handleFieldChange">
          <div class="container">
            ...
            <div class="add-operation">
              ...
              <BaseButton label="Pivot" @add-operation="addOperation('pivot')"/>
            </div>
                ...
                <template v-slot:content>
                  <div class="row">
                    ...
                    <BaseSelect
                      ...
                      :options="filterOptions[columnType(filter.column)]"
                      :required
                      :disabled="noDataFrame"
                      :index="index"
                    />
            ...
            <div class="operations">
              <Accordion
                class="operations-wrapper"
                @remove-operation="removeOperation('pivot')"
                :hasOperations="hasPivot"
              >
                <template v-slot:title>
                  {{ currentPivot }}
                </template>
                <template v-slot:content>
                  <div class="row">
                    ...
                    <div class="input-icon-container grow"
                      ...
                      v-for="(pivot, index) in operations.pivot">
                      <BaseSelect
                        name="pivot_by"
                        operation="pivot"
                        label="Pivot By"
                        v-model="pivot.pivot_by"
                        :options="dataFrameColumns"
                        :required
                        :index="index"
                        :disabled="noDataFrame"
                      />
                      <BaseInputIcon @remove-operation="removeOperation('pivot', index)"/>
                    </div>
                    <template v-else>
                      <BaseSelect
                        name="names_from"
                        label="Names from"
                        v-model="rootFields.names_from"
                        :options="dataFrameColumns"
                        :required
                        :index="index"
                        :disabled="noDataFrame"
                      ...
                      <BaseSelect
                        name="values_from"
                        label="Values from"
                        v-model="rootFields.values_from"
                        :options="dataFrameColumns"
                        :required
                        :index="index"
                        :disabled="noDataFrame"
    ...
    computed: {
      ...
      hasPivot() {
        const pivot = this.operations.pivot;
        return this.rootFields.pivot_type === "pivot_wider"
          ? true
          : pivot.length > 0;
      },
      // noDataFrame() {
    ...
    methods: {
      ...
      columnType(column) {
        ...
        return this.columnTypes[columnType] || "categorical";
      },
    },
  }).mount(ctx.root);
