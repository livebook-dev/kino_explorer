  ...
  const app = Vue.createApp({
        ...
        <form @change="handleFieldChange">
          <div class="container">
            ...
            <div class="add-operation">
              ...
              <BaseButton
                v-if="hasOperations(rootFields.pivot_type) && rootFields.pivot_type === 'pivot_longer'"
                label="Pivot Column"
                @add-operation="addOperation('pivot_longer')"
              />
              <BaseButton
                v-if="!hasOperations(rootFields.pivot_type)"
                label="Pivot"
                @add-operation="addOperation(rootFields.pivot_type)"
              />
            </div>
            <div class="operations" v-if="hasOperations('groups')">
              <Accordion
            ...
            <div class="operations" v-if="hasOperations(rootFields.pivot_type)">
              <Accordion
                class="operations-wrapper"
                @remove-operation="removeOperation(rootFields.pivot_type)"
                :hasOperations="hasOperations(rootFields.pivot_type)"
              >
                <template v-slot:title>
                  {{ currentPivot }}
                </template>
                <template v-slot:content>
                  <div class="row">
                    ...
                    <div class="input-icon-container grow"
                      v-if="rootFields.pivot_type === 'pivot_longer'"
                      v-for="(pivot_longer, index) in operations.pivot_longer">
                      <BaseSelect
                        name="pivot_by"
                        operation="pivot_longer"
                        label="Pivot By"
                        v-model="pivot_longer.pivot_by"
                        :options="dataFrameColumns"
                        :required
                        :index="index"
                        :disabled="noDataFrame"
                        selectClass="input input-icon"
                      />
                      <BaseInputIcon @remove-operation="removeOperation('pivot_longer', index)"/>
                    </div>
                    <template v-else v-for="(pivot_wider, index) in operations.pivot_wider">
                      <BaseSelect
                        name="names_from"
    ...
    data() {
      return {
        ...
        pivotOptions: [
          { label: "longer", value: "pivot_longer" },
          { label: "wider", value: "pivot_wider" },
        ],
        filterOptions: {
          numeric: [
    ...
    computed: {
      ...
      currentGroups() {
        const groups = this.operations.groups
          .map(({ group_by }) => group_by)
          .join(", ");
