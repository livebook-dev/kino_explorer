export async function init(ctx, payload) {
  ...
  await importJS(
    "https://cdn.jsdelivr.net/npm/@vueform/multiselect@2.6.1/dist/multiselect.global.min.js"
  );
  ctx.importCSS(
    "https://fonts.googleapis.com/css2?family=Inter:wght@400;500&display=swap"
  );
  ctx.importCSS(
    "https://cdn.jsdelivr.net/npm/@vueform/multiselect@2.6.1/themes/default.min.css"
  );
  ...
  const app = Vue.createApp({
                    ...
                    >
                      ...
                      <template v-slot:content>
                        ...
                        <div class="row" v-if="operation.operation_type === 'pivot_wider'">
                          ...
                          />
                          <div>
                            <span class="input-label">Values from</span>
                            <Multiselect
                              ...
                              :index="index"
                              :disabled="!operation.names_from"
                              mode="tags"
                              class="multiselect multiselect-pivot-wider"
                              noResultsText=""
                              noOptionsText=""
                              :class="{'is-empty': dataFrameColumnsByType('integer').length === 0}"
                            />
                          </div>
                        </div>
                      </template>
    ...
    methods: {
      ...
      handleMultiSelectChange(value, event) {
        console.error(value)
        const {operation_type, index, field} = Object.assign({}, event._.attrs);
        ctx.pushEvent("update_field", {
          operation_type,
          field,
          value,
